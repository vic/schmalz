...
main() ->
  ...
  io:fwrite("Schmalz (Story: ~s) Copyright (C) 2007 Wei-ju Wu~n"
...
start(Filename) ->
    MachineState = machine:create(memory:read_file(Filename)),
    MachinePid = spawn(fun() -> machine:listen(MachineState) end), 
    run(MachinePid, 1).
