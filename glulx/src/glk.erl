...
-record(glk_window, {id, type, size, rock, buffer}).

init() ->
    GlkState = #glk_state{windows = [], streams = [], filerefs = []},
...
window_open(#glk_state{windows = []} = GlkState0,
	    [0, _, _, Wintype, Rock]) ->
    io:format("OPENING INITIAL WINDOW, "
	      "WinType: ~w, Rock: ~w~n", [Wintype, Rock]),
    Window = #glk_window{id = 1, type = Wintype, size = ?INITIAL_WINDOW_SIZE,
			 rock = Rock, buffer = []},
    {{glk_result, Window#glk_window.id, []},
     GlkState0#glk_state{windows = [Window]}};
window_open(_GlkState0, [Split, Method, Size, Wintype, Rock]) ->
